...
def assess_quality(price_data: dict) -> dict:
    ...
    if merchant_count is None:
        # Legacy/provider rows may omit it — derive distinct merchant domains.
        domains = set()
        for src in price_data.get("sources") or []:
            try:
        ...
        merchant_count = len(domains)
    # Evidence = independent price offers backing the estimate. An aggregator that
    # lists many merchant offers is real market evidence, so we score on the offer
    # count, falling back to distinct sources for legacy rows that predate it.
    raw_offers = price_data.get("offer_count")
    evidence = raw_offers if isinstance(raw_offers, int) else source_count
    ...
    # Base tier from how many independent price offers back the estimate.
    if evidence == 0:
        tier, score, label = "unsourced", 0, "Unsourced estimate"
    elif evidence == 1:
        tier, score, label = "single", 1, "Single listing"
    elif evidence == 2:
        tier, score, label = "limited", 2, "Limited data"
    else:
        tier, score, label = "multiple", 3, "Multiple listings"

    # Web-parsed prices never read as authoritative; "moderate" is the ceiling, and
    # any weakening factor (single point, stale, very wide spread, or a single
    # aggregator domain) pulls confidence back down to "low".
    confidence = "moderate" if score >= 3 else "low"
    if is_single_point or is_stale or merchant_count <= 1 or (spread_pct is not None and spread_pct > WIDE_SPREAD_PCT):
        confidence = "low"

    if evidence == 0:
        note = "No merchant listing was captured for this price."
    else:
        plural = "s" if evidence != 1 else ""
        note = f"Based on {evidence} public listing{plural}."
    if merchant_count <= 1 and evidence > 1:
        note += " All via a single aggregator, so treat as indicative, not independent."
    if is_single_point and evidence:
